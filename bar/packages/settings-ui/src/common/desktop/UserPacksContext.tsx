...
import type { Widget, WidgetConfig, WidgetPack } from 'ninja';
...
/// A widget together with the pack that stores it.
///
/// Packs remain the on-disk storage unit, but the UI presents a single
/// flat list of widgets. `packId` is carried along only because it's
/// still needed to address the widget in commands and routes.
export type FlatWidget = {
  packId: string;
  widget: WidgetConfig;
};

type UserPacksContextState = {
  allWidgets: Resource<FlatWidget[]>;
...
export function UserPacksProvider(props: { children: JSX.Element }) {
  ...
  const [allWidgets] = createResource(allPacks, packs =>
    (packs ?? []).flatMap(pack =>
      pack.widgets.map(widget => ({ packId: pack.id, widget })),
    ),
  );
