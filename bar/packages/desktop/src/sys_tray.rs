...
/// System tray icon for Ninja.
///
/// Currently runs without an icon: the window manager shares this process
/// and owns the single tray icon. The menu-building code is kept so the
/// icon can be restored by constructing one again in `new`.
#[allow(dead_code)]
pub struct SysTray {
...
#[allow(dead_code)]
impl SysTray {
  /// Creates a new system tray icon for Ninja.
  pub async fn new(
    app_handle: &AppHandle,
    app_settings: Arc<AppSettings>,
    widget_pack_manager: Arc<WidgetPackManager>,
    ...
    // No icon of its own: the window manager shares this process and puts
    // up the single tray icon for the whole app. Two icons for one program
    // is worse than one, and the items this menu offers are all reachable
    // from the settings window that tray opens.
    //
    // The rest of the type is still live — `refresh` is a no-op without an
    // icon, and re-adding one here is all it would take to bring the menu
    // back.
    Ok(sys_tray)
  }

  async fn create_tray_icon(&self) -> anyhow::Result<TrayIcon> {
    let tooltip = format!("Ninja Bar v{}", VERSION_NUMBER);
    ...
    let tray_icon = tray_icon.build(&self.app_handle)?;

    // Hidden: the window manager owns the single tray icon for the
    // product, and drives this process through its CLI instead. The icon
    // is still built so that menu state stays in sync if it is ever shown.
