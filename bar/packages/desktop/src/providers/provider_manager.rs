...
use super::{
  ...
  ninja::NinjaProvider, Provider, ProviderConfig, ProviderFunction,
...
impl ProviderManager {
  ...
  pub async fn create(
    ...
    // A provider whose task has ended is a dead entry. Evict it, along
    // with whatever it last emitted, so that a live one is created below.
    //
    // Without this, a provider that gave up stayed "created" forever and
    // every widget window opened afterwards replayed its final error. The
    // WM provider used to exit on a failed connection, which is how a
    // 100ms startup race left the workspace indicators blank for days.
    if provider_refs
      .get(&config_hash)
      .is_some_and(|existing| existing.task_handle.is_finished())
    {
      tracing::info!("Replacing stopped provider: {}", config_hash);

      provider_refs.remove(&config_hash);
      self.emit_cache.lock().await.remove(&config_hash);
    }

    // If a live provider with the given config already exists, re-emit its
    // latest emission and return early. Multiple frontend clients can call
    // `create` for the same provider, and all will receive the same output
    // once the provider emits.
  ...
  fn create_instance(
    ...
    let runtime_type = match config {
      ProviderConfig::Ninja(..) => RuntimeType::Async,
      #[cfg(any(target_os = "macos", windows))]
    ...
    let task_handle = match &runtime_type {
      RuntimeType::Async => task::spawn(async move {
        match config {
          ProviderConfig::Ninja(config) => {
            let mut provider = NinjaProvider::new(config, common);
