...
use crate::{
  common::{
    to_iec_bytes, to_si_bytes, SyncInterval, MIN_REFRESH_INTERVAL_MS,
  },
  providers::{
...
impl NetworkProviderConfig {
  /// Refresh interval actually used by the timer.
  ///
  /// `SyncInterval` clamps the configured value, so the raw config value
  /// can be zero. Per-second figures must be derived from the clamped
  /// interval to match reality and to avoid a divide-by-zero.
  fn clamped_refresh_interval(&self) -> u64 {
    self.refresh_interval.max(MIN_REFRESH_INTERVAL_MS)
  }
}
...
impl NetworkProvider {
  ...
  fn run_interval(&mut self) -> anyhow::Result<NetworkOutput> {
    ...
    let refresh_interval = self.config.clamped_refresh_interval();

    let (received, total_received) = Self::bytes_received(&self.netinfo);
    let received_per_sec = received / refresh_interval * 1000;

    let (transmitted, total_transmitted) =
      Self::bytes_transmitted(&self.netinfo);
    let transmitted_per_sec = transmitted / refresh_interval * 1000;
...
mod tests {
  ...
  /// A zero refresh interval must not reach the per-second division.
  #[test]
  fn zero_refresh_interval_is_clamped() {
    ...
    assert_eq!(config.clamped_refresh_interval(), MIN_REFRESH_INTERVAL_MS);
  }

  /// A configured interval above the minimum is used as-is.
  #[test]
  fn normal_refresh_interval_is_untouched() {
    let config = NetworkProviderConfig {
      refresh_interval: 5000,
    };

    assert_eq!(config.clamped_refresh_interval(), 5000);
  }
}
