...
/// Manages the creation of Ninja widgets.
#[derive(Debug)]
...
impl WidgetFactory {
  ...
  pub async fn start_widget_by_pack(
    ...
    for coordinates in self.widget_coordinates(placement).await {
      ...
      .title(format!("Ninja - {} / {}", widget_pack.id, widget_name))
  ...
  fn initialization_script(
    ...
    // Also published under the old name, so a widget bundling the
    // upstream client API still finds its state.
    let state_script = format!(
      "window.__NINJA_STATE={0};window.__ZEBAR_STATE=window.__NINJA_STATE;",
