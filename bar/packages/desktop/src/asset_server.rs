...
/// Cookie carrying a widget's asset server token.
const TOKEN_COOKIE: &str = "NINJA_TOKEN";

/// Cookie name used before the rename, still read so a widget window left
/// open across an upgrade keeps its access.
const LEGACY_TOKEN_COOKIE: &str = "ZEBAR_TOKEN";
  ...
  let rocket = rocket::build()
    .configure(
      rocket::Config::figment()
        ...
        // Bound explicitly to loopback. Rocket's default address is
        // `127.0.0.1` in debug but `0.0.0.0` in release, so without this
        // the asset server is exposed to the whole local network in
        // shipped builds.
...
pub async fn create_init_url(
  ...
  let url = tauri::Url::parse_with_params(
    &format!("http://127.0.0.1:{}/__ninja/init", ASSET_SERVER_PORT),
...
#[get("/__ninja/init?<token>&<redirect>")]
pub fn init(
  token: String,
  redirect: String,
  cookies: &CookieJar<'_>,
...
/// Serves the client API to widgets.
///
/// Widgets import this instead of fetching the published package from a
/// CDN. That keeps the client in step with this build of the desktop app
/// (which talks to the WM over a pipe, not a websocket), and means a
/// widget needs no network access to start.
///
/// This is the `dist-browser` bundle, which has every dependency inlined.
/// Widgets load it as a plain module with no bundler, so the regular
/// package build's bare imports would fail to resolve.
#[get("/__ninja/client.js")]
pub fn client_js() -> (ContentType, &'static str) {
  (
    ContentType::JavaScript,
    include_str!("../../client-api/dist-browser/client.js"),
  )
}
...
#[get("/__ninja/normalize.css")]
pub fn normalize_css() -> (ContentType, &'static str) {
  (ContentType::CSS, include_str!("../resources/normalize.css"))
}

// The `/__zebar` routes below are kept so widget packs written against the
// upstream bar keep working. They serve exactly what the `/__ninja` routes
// serve; nothing new should be added here.
...
pub fn legacy_init(
  ...
  init(token, redirect, cookies)
}

#[get("/__zebar/client.js")]
pub fn legacy_client_js() -> (ContentType, &'static str) {
  client_js()
}

#[get("/__zebar/sw.js")]
pub fn legacy_sw_js() -> SwResponse {
  sw_js()
}

#[get("/__zebar/normalize.css")]
pub fn legacy_normalize_css() -> (ContentType, &'static str) {
  normalize_css()
}
...
impl<'r> FromRequest<'r> for ServerToken {
  ...
  async fn from_request(
    ...
    let token = request
      .cookies()
      .get(TOKEN_COOKIE)
      .or_else(|| request.cookies().get(LEGACY_TOKEN_COOKIE));
