...
impl AppSettings {
  ...
  pub fn new(
    ...
    let webview_cache_dir = app_handle
      .path()
      .resolve("bar/webview-cache", BaseDirectory::Data)
    ...
    let marketplace_download_dir = app_handle
      .path()
      .resolve("bar/downloads", BaseDirectory::Data)
      .context("Unable to resolve app data directory.")?;

    let migration_file = app_handle
      .path()
      .resolve("bar/.migrations.json", BaseDirectory::Data)
  ...
  /// `with-ninja` from the built-in starter widget pack as
  /// startup config.
  fn create_default(config_dir: &Path) -> anyhow::Result<()> {
    ...
    let default_settings = AppSettingsValue {
      ...
      startup_configs: vec![StartupConfig {
        ...
        widget: match is_app_installed("ninja") {
          true => "with-ninja".into(),
          false => "vanilla".into(),
        },
        preset: "default".into(),
      }],
  ...
  pub async fn remove_startup_config(
    ...
    new_settings.startup_configs.retain(|config| {
      config.pack != pack_id
        || widget_name.is_some_and(|w| config.widget != w)
        || preset_name.is_some_and(|p| config.preset != p)
    });
