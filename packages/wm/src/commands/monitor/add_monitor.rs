...
/// Attaches every workspace bound to the given monitor via
/// `bind_to_monitor`, moving those that are already active elsewhere and
/// activating those that aren't.
///
/// Should only be called once the monitors have been sorted, since bound
/// workspaces are matched against the monitor's index.
pub fn move_bounded_workspaces_to_new_monitor(
  ...
  for workspace_config in bound_workspace_configs {
    ...
    if let Some(existing_workspace) = existing_workspace {
      ...
      // Recreate workspaces bound to this monitor that aren't currently
      // active. Bound workspaces are always materialized, so that they
      // survive the monitor being disconnected and reconnected (e.g. a
      // DisplayPort monitor powering down on screen timeout).
      activate_workspace(
