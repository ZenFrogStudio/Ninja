...
pub fn handle_window_focused(
  ...
  if let Some(window) = found_window {
    ...
    if window.display_state() == DisplayState::Hidden {
      ...
      // Switching workspace here is a reaction to the OS focusing this
      // window, not a switch the user asked for: clicking the app's
      // taskbar button is the usual way to get here. `focus_workspace`
      // queues a cursor jump for the keyboard-driven case, which would
      // yank the cursor to the target monitor mid-click.
