  ...
  if let Some(window) = found_window {
    ...
    if is_minimized && window.state() != WindowState::Minimized {
      ...
      if let Some(focus_target) = state.focus_target_after_removal(&window)
      {
        ...
        // No cursor jump: focus lands here as fallout from the window
        // going away, not because the user aimed at another monitor. The
        // usual way to get here is clicking an app's taskbar button a
        // second time, which would throw the cursor off the taskbar and
        // onto whichever monitor inherited focus.
