...
impl WmState {
  ...
  pub fn populate(
    ...
    // Reclaim windows stranded by a previous session before enumerating,
    // since a cloaked window is absent from `visible_windows`.
    #[cfg(target_os = "windows")]
    self.uncloak_stranded_windows();
...
impl WmState {
  /// Uncloaks windows left cloaked by a previous WM session.
  ///
  /// A session that ends without uncloaking what it hid strands those
  /// windows for good: they're absent from the taskbar and the task
  /// switcher, and the next session can't reclaim them either, because a
  /// cloaked window reports itself as not visible. Restoring on exit is
  /// what should prevent this; this is the backstop for the exits that
  /// don't get to run, such as a power loss.
  ///
  /// The shell cloaks windows of its own accord too — suspended UWP apps,
  /// mainly — so candidates are filtered down to those that look like
  /// ordinary application windows first. Anything uncloaked here still has
  /// to pass the usual checks in `manage_window` to actually be managed;
  /// the worst case for a bad guess is a window the WM ignores becoming
  /// visible, which the shell undoes on its own.
  #[cfg(target_os = "windows")]
  fn uncloak_stranded_windows(&self) {
    use wm_platform::{
      NativeWindowWindowsExt, WS_CHILD, WS_EX_NOACTIVATE, WS_EX_TOOLWINDOW,
    };

    let Ok(cloaked_windows) = self.dispatcher.cloaked_windows() else {
      return;
    };

    for window in cloaked_windows {
      let is_ordinary_window = !window.has_window_style(WS_CHILD)
        && !window
          .has_window_style_ex(WS_EX_NOACTIVATE | WS_EX_TOOLWINDOW)
        && window.title().is_ok_and(|title| !title.is_empty());

      if !is_ordinary_window {
        continue;
      }
      ...
      match window.set_cloaked(false) {
        Ok(()) => {
          tracing::info!(
            "Reclaimed stranded window: {:?}",
            window.title()
          );
        }
        Err(err) => {
          warn!("Failed to reclaim stranded window: {:?}", err);
        }
      }
    }
  }

  /// Returns every managed window to a state the user can reach.
  ///
  /// Safe to call more than once: every operation sets an absolute state
  /// rather than toggling one.
  ///
  /// Windows are made reachable before they're made pretty. Uncloaking is
  /// what rescues a window — a cloaked window is absent from both the
  /// taskbar and the task switcher — while borders and transparency are
  /// cosmetic. The COM calls involved block with no timeout when the
  /// owning process is unresponsive, so an early one that hangs must not
  /// be what strands the rest.
  pub fn restore_windows(&self) {
    let managed_windows = self.windows();

    // Cheap and non-blocking, so it runs for every window first.
    ...
    #[cfg(target_os = "windows")]
    for window in &managed_windows {
      // Undoes `hide_method: 'cloak'`. `show` above only undoes an
      // `SW_HIDE` from `hide_method: 'hide'`; cloaking is a separate
      // mechanism and needs its own reversal, without which the window
      // survives the WM but can never be found again.
