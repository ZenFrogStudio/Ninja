...
use crate::{
  commands::{
    ...
    workspace::{
      add_workspace, focus_workspace, move_workspace_in_direction,
...
impl WindowManager {
  ...
  pub fn run_command(
    ...
    match &command {
      InvokeCommand::AddWorkspace { name } => {
        add_workspace(name.as_deref(), &subject_container, state, config)
      }
  ...
  pub(crate) fn cleanup(
    ...
    // Restore windows before announcing the exit, not after. The watchdog
    // stands down the moment it sees `ApplicationExiting`, on the
    // assumption that the WM has cleaned up after itself — so anything
    // done afterwards has no safety net if the WM dies partway through.
    self.state.restore_windows();
