...
impl PendingSync {
  ...
  /// Cancels a cursor jump queued earlier in this sync.
  ///
  /// For focus changes the user didn't ask for directly. A cursor jump
  /// exists so a keyboard-driven focus change doesn't strand the cursor on
  /// the monitor it came from; when the OS moved focus because the user
  /// clicked something, the cursor is already where they put it.
