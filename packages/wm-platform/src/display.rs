...
pub trait DisplayExtWindows {
  /// Gets the monitor handle (`HMONITOR`) as a raw value.
  ///
  /// Returned as an `isize` rather than the `HMONITOR` itself, so that the
  /// `windows` crate's types stay inside this crate.
  ///
  /// # Platform-specific
  ///
...
impl DisplayExtWindows for Display {
  fn hmonitor(&self) -> isize {
    self.inner.hmonitor().0 as isize
  }
}
