...
impl DispatcherExtWindows for Dispatcher {
  ...
  fn expand_env_strings(&self, input: &str) -> crate::Result<String> {
    ...
    // The second call must be checked as well as the first. A zero here
    // would otherwise underflow the subtraction below and panic, which is
    // reachable from any caller that controls the input string.
...
impl Dispatcher {
  ...
  /// Gets windows that are shown but cloaked.
  ///
  /// NOTE: Does not guarantee a specific, consistent order.
  ///
  /// These are invisible to [`Dispatcher::visible_windows`], since a
  /// cloaked window reports itself as not visible. A WM session that ended
  /// without uncloaking what it hid leaves its windows here, unreachable
  /// from both the taskbar and the task switcher.
  ///
  /// The result also includes windows the shell cloaks for its own
  /// reasons, such as suspended UWP apps, so callers are expected to
  /// filter it.
  ///
  /// # Platform-specific
  ///
  /// - **macOS**: Returns an empty vector. Cloaking is a Windows concept.
  pub fn cloaked_windows(&self) -> crate::Result<Vec<NativeWindow>> {
    #[cfg(target_os = "macos")]
    {
      Ok(Vec::new())
    }
