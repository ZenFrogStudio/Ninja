...
pub(crate) struct WindowListener {
  /// Held as `isize` rather than `HWINEVENTHOOK`, which wraps a raw
  /// pointer and so can't cross the thread boundary the hooks are set on.
  hook_handles: Vec<isize>,
}
...
impl WindowListener {
  ...
  pub(crate) fn new(
    ...
    // Handles come back as `isize`, since `HWINEVENTHOOK` wraps a raw
    // pointer and so isn't `Send`.
    let hook_handles = dispatcher.dispatch_sync(move || {
      ...
      Self::hook_win_events().map(|handles| {
        handles
          .into_iter()
  ...
  extern "system" fn window_event_proc(
    ...
    // than a UI control.
    let is_window_event = id_object == OBJID_WINDOW.0
      && id_child == 0
    ...
    let event = match event_type {
      EVENT_OBJECT_DESTROY => WindowEvent::Destroyed {
        window_id: WindowId(handle.0 as isize),
        notification,
      },
      EVENT_SYSTEM_FOREGROUND => WindowEvent::Focused {
        window: NativeWindow::new(handle.0 as isize).into(),
        notification,
      },
