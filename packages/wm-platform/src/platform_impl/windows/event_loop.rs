...
impl EventLoopSource {
  ...
  pub(crate) fn send_stop(&self) -> crate::Result<()> {
    // This is the only place in the application that posts `WM_QUIT`. If
    // the event loop returns without this having been logged, the quit
...
impl EventLoop {
  ...
  fn create_message_window(
    ...
    // `CreateWindowExW` reports failure through its `Result` rather than a
    // null handle since `windows` 0.58.
  ...
  unsafe extern "system" fn window_proc(
    ...
    let handled = WNDPROC_CALLBACKS.with(|cbs| {
      for callback in cbs.borrow().values() {
        if let Some(result) =
          callback(hwnd.0 as isize, msg, wparam.0, lparam.0)
        {
