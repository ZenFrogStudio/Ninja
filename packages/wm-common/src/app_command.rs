...
#[clap(name = "ninja", author, version = VERSION, about, long_about = None)]
pub enum AppCommand {
  /// Starts the window manager.
  Start {
    /// Custom path to user config file.
    ///
    /// The default path is `%userprofile%/.ninja/config.yaml`
...
///
/// The default is neither verbose nor quiet, matching a bare invocation
/// with no flags.
...
pub enum QueryCommand {
  ...
  AppMetadata,
  /// Outputs the path of the config file currently in use.
  ///
  /// Reflects the `--config` override when one was given, so tooling
  /// edits the same file the WM loaded.
  ConfigPath,
...
pub enum InvokeCommand {
  AddWorkspace {
    ...
    ///
    /// Defaults to the lowest unused positive integer.
